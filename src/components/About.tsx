...
export function About() {
  const facts = [
    ...
    { label: "Founding Year", value: "1998" },
    { label: "Headquarters", value: "Ludhiana, Punjab" },
    { label: "Manufacturing", value: "45,000+ Sq. Ft." },
    { label: "Production", value: "5,000+ Units/Year" },
    { label: "Certifications", value: "BIS & ISO 9001" },
    { label: "Market Reach", value: "Pan-India & Global" },
  ];

  const features = [
    { icon: ShieldCheck, title: "Boron Steel", desc: "Hardened blades for maximum soil wear resistance." },
    { icon: Factory, title: "CNC Precision", desc: "Automated machining for vibration-free gearboxes." },
    { icon: Truck, title: "Swift Logistics", desc: "Daily transport to Haryana, UP, and Rajasthan." },
  ];

  const milestones = [
    { year: "1998", title: "Foundation", desc: "SB AGROTECH was founded in Ludhiana with a vision to revolutionize soil preparation.", icon: History },
    { year: "2005", title: "Boron Revolution", desc: "Pioneered the use of high-grade Boron steel for rotavator blades in the Punjab market.", icon: Star },
    { year: "2012", title: "Global Standards", desc: "Achieved ISO 9001:2015 certification and expanded to a 45,000 sq. ft. facility.", icon: CheckCircle2 },
    { year: "2018", title: "Export Division", desc: "Started exporting machinery to international markets including Africa and SE Asia.", icon: Globe },
    { year: "2024", title: "Tech Integration", desc: "Launched Gear Concierge and digitized our supply chain for zero-downtime support.", icon: TrendingUp },
  ];
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            <div className="animate-fade-in-up order-2 lg:order-1">
              <span className="text-secondary font-bold tracking-widest uppercase text-xs md:text-sm mb-4 block animate-pulse">Our Legacy</span>
              ...
              <p className="text-muted-foreground text-base md:text-lg mb-8 leading-relaxed">
                Established in 1998 in Ludhiana, SB AGROTECH has over two decades of experience engineering tools for the Indo-Gangetic plains. Our machinery is tested in the toughest clay soils to ensure it stands up to multi-crop cycles.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-10">
                {features.map((f, i) => (
                  <div key={i} className="flex flex-col items-center sm:items-start text-center sm:text-left group cursor-default">
                    ...
                    <h4 className="font-bold text-primary mb-2 text-lg group-hover:text-secondary transition-colors">{f.title}</h4>
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm border border-border flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left transition-all hover:shadow-md hover:border-primary/20 cursor-default">
                ...
                <p className="text-primary font-semibold text-sm md:text-base">
                  BIS & ISO 9001:2015 Certified Manufacturing Facility
                </p>
              </div>
            </div>
            ...
            <div className="animate-fade-in-up animate-delay-200 order-1 lg:order-2">
              <Card className="border-none shadow-2xl overflow-hidden rounded-[2.5rem] transition-transform hover:scale-[1.02] duration-500">
                <div className="bg-primary p-6 md:p-10 relative overflow-hidden">
                  ...
                  <h3 className="text-white text-2xl md:text-3xl font-headline font-bold relative z-10">Business Factsheet</h3>
                  <p className="text-white/70 text-sm mt-2 relative z-10">Official stats for B2B Dealers & Importers</p>
                </div>
                <CardContent className="p-0">
                  <div className="grid grid-cols-1 divide-y divide-border">
                    {facts.map((fact, i) => (
                      <div key={i} className="grid grid-cols-2 p-4 md:p-6 hover:bg-muted/50 transition-all duration-300 group">
                        <span className="text-muted-foreground font-medium text-sm md:text-base group-hover:text-primary transition-colors">{fact.label}</span>
                        <span className="text-primary font-bold text-sm md:text-base text-right group-hover:scale-105 transition-transform">{fact.value}</span>
                      </div>
                    ))}
                  </div>
                  <div className="p-6 md:p-8 bg-muted/50 border-t">
                    <h4 className="font-bold text-xs uppercase tracking-widest text-muted-foreground mb-4">Trading & Support</h4>
                    <div className="flex flex-wrap gap-2">
                      {["RTGS/NEFT", "Spare Parts Hub", "On-Field Demo", "Expert Calibration"].map((tag) => (
      ...
      <section className="bg-muted/30 py-20 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-8">
          <div className="text-center mb-16 animate-fade-in-up">
            <span className="text-secondary font-bold tracking-widest uppercase text-xs md:text-sm mb-4 block">Our Journey</span>
            <h2 className="text-3xl md:text-5xl font-headline font-bold text-primary mb-6">Milestones of Excellence</h2>
